...
let handler = async (m, {conn, args}) => {
  ...
  if (/image/.test(mime)) {
    ...
    await conn.updateProfilePicture(m.chat, {url}).then((_) => m.reply("_. ᩭ✎Imagen actualizada con éxito._"));
  } else if (args[0] && args[0].match(URL_REGEX)) {
    await conn.updateProfilePicture(m.chat, {url: args[0]}).then((_) => m.reply("_. ᩭ✎Imagen actualizada con éxito._"));
  } else throw "_. ᩭ✎Responde a una imagen_";
};
