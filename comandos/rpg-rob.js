...
let handler = async (m, {conn, text, usedPrefix, command, groupMetadata}) => {
  ...
  if (new Date() - global.db.data.users[m.sender].lastrob < 7200000) throw `_. ᩭ✎Espera ${msToTime(time - new Date())} para volver a robar_`;
  if (!text) return m.reply(`_. ᩭ✎Etiquetɑ ɑl usuɑrio que quierɑ sɑqueɑr_\n\n_. ᩭ✎Ejemplo de uso:* ${usedPrefix + command} @tɑg._`);
  try {
    let _user = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted.sender;
    if (!_user in global.db.data.users) return m.reply(`_. ᩭ✎El usuɑrio no estά registrɑdo en lɑ bɑse de dɑtos_`);
    if (m.quoted?.sender) m.mentionedJid.push(m.quoted.sender);
    if (!m.mentionedJid.length) m.mentionedJid.push(m.sender);
    if (global.db.data.users[_user] == undefined) return m.reply(`_. ᩭ✎El usuɑrio no estά registrɑdo en lɑ bɑse de dɑtos_`);
    if (_user.startsWith(conn.user.jid.split`@`[0])) return m.reply("_. ᩭ✎No puedes saquear a la bot :/_");
    let uuser = global.db.data.users[_user];
    let dolares = Math.floor(Math.random() * 15) + 10;
    let limit = Math.floor(Math.random() * 5) + 3;
    let raid = `_. ᩭ✎Has saqueado ⚔️ a @${_user.split("@s.whatsapp.net")[0]}_
_. ᩭ✎Dinero: $${dolares}_
_. ᩭ✎Diamante: ${limit}_

_. ᩭ✎Robado por: @${m.sender.split("@")[0]}_`;
    if (uuser.limit <= 5) return m.reply("_. ᩭ✎El usuario no tiene suficientes recursos_");
    if (uuser.dolares <= 10) return m.reply(`_. ᩭ✎El usuario no tiene suficientes recursos_`);
    ...
    await m.reply(`_. ᩭ✎No pudiste Robar a @${_user.split("@s.whatsapp.net")[0]} porque al usuario lo protege la policía (Afk)_`);
  }
};
