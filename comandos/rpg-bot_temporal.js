...
conn.sendMessage(m.chat, { text: dorratunido2 }, { quoted: m }) 
    ...
    if (user.joincount === 0)
      throw `${ag}*¡YA NO TIENES TOKENS! 🪙*\n\n*COMPRA TOKENS PARA PODER INVITAR A NIGHTCORE-BOT A TÚ GRUPO CON EL COMANDO *${usedPrefix}buy joincount 3 o ${usedPrefix}buy Para ver la Tienda*`;
    if (!args[1])
    ...
    let res = await conn.groupAcceptInvite(code);
    conn.reply(m.chat, `✅ *SE HA UNIDO NIGHTCORE-BOT AL GRUPO!!!*`, m).then(async () => {
      ...
      await conn.reply(
        m.chat,
        `${wm} *ES UN BOT DE WHATSAPP QUE TE AYUDARÁ A REALIZAR DIFERENTES ACTIVIDADES 🪄 AL PRIVADO O GRUPO Y TAMBIÉN TE VAS A DIVERTIR 🎈 CON SUS MULTIPLES FUNCIONES, DISFRUTA DE NIGHTCORE-BOT!!! 😎*\n\n👑 *NIGHTCORE-BOT FUE INVITADO POR:*\n${username}`,
        m
      );
