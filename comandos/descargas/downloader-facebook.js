import fg from 'api-dylux'  
 ...
 if (!args[0]) throw `*✘error✘* _. ᩭ✎Use el comandó correctamente_\n\n_Ejemplo : ${usedPrefix + command} https://fb.watch/fOTp0n6UFQ/ 🍁_`  
 if (!args[0].match(/www.facebook.com|fb.watch/g)) throw `*✘error✘* _. ᩭ✎Use el comandó correctamente_\n\n_Ejemplo : ${usedPrefix + command} https://fb.watch/fOTpgn0UFQ/ 🍁_` 
 try { 
 await m.reply(`ᴅᴇsᴄᴀʀɢᴀɴᴅᴏ ᴠɪᴅᴇᴏ, ᴀɢᴜᴀʀᴅᴇ ᴜɴ ᴍᴏᴍᴇɴᴛᴏ, ᴇsᴛᴇ ᴘʀᴏᴄᴇsᴏ ᴘᴜᴇᴅᴇ ᴅᴜʀᴀʀ ᴇɴᴛʀᴇ 1 ʏ 2 ᴍɪɴᴜᴛᴏs ᴅᴇᴘᴇɴᴅɪᴇɴᴅᴏ ᴅᴇ ʟᴀ ᴅᴜʀᴀᴄɪᴏɴ ᴅᴇʟ ᴠɪᴅᴇᴏ...`)       
 const d2ata = await facebook.v1(args[0]); 
 let r2es = ''; 
 if (d2ata.urls && d2ata.urls.length > 0) { 
 r2es = `${d2ata.urls[0]?.hd || d2ata.urls[1]?.sd || ''}`} 
await  conn.sendFile(m.chat, r2es, 'error.mp4', `*◈⏤͟͟͞͞Listo⏤͟͟͞͞◈*`, m); 
 } catch (err1) { 
 try {   
...
await  conn.sendFile(m.chat, VIDEO, 'error.mp4', `*◈⏤͟͟͞͞Listo⏤͟͟͞͞◈*`, m)     
 } catch (err2) {     
 try { 
 let ress = await fg.fbdl(args[0]) 
 let urll = await ress.data[0].url     
 await conn.sendFile(m.chat, urll, 'error.mp4', '*◈⏤͟͟͞͞Listo⏤͟͟͞͞◈*', m)      
 } catch (err3) { 
 try { 
 let res = await fbDownloader(args[0]) 
 for (let result of res.download) { 
 let ur = result.url     
 await conn.sendFile(m.chat, ur, 'error.mp4', '*◈⏤͟͟͞͞Listo⏤͟͟͞͞◈*', m)} 
 } catch (err4) { 
 try { 
 let res3 = await fetch(`https://latam-api.vercel.app/api/facebookdl?apikey=brunosobrino&q=${args[0]}`)   
 let json = await res3.json() 
 let url3 = await json.video 
 await conn.sendFile(m.chat, url3, 'error.mp4', '*◈⏤͟͟͞͞Listo⏤͟͟͞͞◈*', m)          
 } catch (err5) {     
 try { 
