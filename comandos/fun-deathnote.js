...
let handler = async (a, {conn: n, text: t, participants: r, usedPrefix, command}) => {
  if (!t)
    return a.reply(`_. ᩭ✎Agrege una razon para la ejecucion_
_Ejemplo:_

_${usedPrefix + command} Hoy muere un random :v_
`);
  if (t.length < 9) return a.reply("_. ᩭ✎La razon es muy corta_");
  ...
  if (m.startsWith(n.user.id.split(":")[0])) return a.reply("_. ᩭ✎Hoy no muere nadie :D_");
  n.sendMessage(
    a.chat,
    {
      text: `*[ Death Note ]* 📓
...
_. ᩭ✎Tiene ${(o % 6e4) / 1e3} segundos para decir sus ultimas palabras_
`,
      mentions: [m],
    },
    {
